...
CodeGen.generateCode = function(ast){
	...
	CodeGen.jumpStart = -1;
	CodeGen.jumpNum = 0;
	var currentScope = SemanticAnalyser.lastScope;
	objectifyScope();
	var scopeNum = -1;
	var varNum = 0;
	//reset
...
varLookup = function(variable, scopeNum){
	while (CodeGen.scopes[scopeNum][variable] === undefined){
		if (scopeNum === undefined || CodeGen.scopes[scopeNum]['parent'] === undefined){
			break;
		}
		scopeNum = CodeGen.scopes[scopeNum]['parent'];
	}
	return CodeGen.scopes[scopeNum][variable];
}
...
objectifyScope = function(){
	CodeGen.scopes = {};
	var parentScope = SemanticAnalyser.lastScope;

	var traverseScopes = function(scope){
		scope.vars['parent'] = scope.parent.num;
		CodeGen.scopes[scope.num] = scope.vars;
